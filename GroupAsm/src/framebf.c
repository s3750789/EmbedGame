...
char* cell_name[48] = {};			//Name of the cell



void define_cell(){
	int step_width = WIDTH_SIZE/8;
	int step_height = HEIGHT_SIZE/6;

	int cell_ind = 0;
	for (int h = 0 ; h < HEIGHT_SIZE ; h=h+step_height){
		for (int w = 0 ; w < WIDTH_SIZE; w=w+step_width){
			cell_coor[cell_ind][0] = w;	//store coordinate width
			cell_coor[cell_ind][1] = h;	//store coordinate height
			cell_ind++;
		}
	}
